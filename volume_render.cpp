...
/*-------------------- User Control -----------------*/
// ...
float rAngle = 0.f; //rotation angle
float iVal = 0.2; //iso-value
GLuint textISNId;
/* -------------------------------------------------------- */
...
void mainRender()
{
	...
	// ...
	GLuint TexISNID = glGetUniformLocation(g_glslProgram, "texISN");
	glActiveTexture(GL_TEXTURE0+1);
	glBindTexture(GL_TEXTURE_2D, textISNId);
	glUniform1i(TexISNID, 1);
	
	// Send the user-controled variables
	// ...
	
	//Send Rotation angle [1]
	GLint anglLoc = glGetUniformLocation(g_glslProgram, "alpha");
	glUniform1f(anglLoc, rAngle);
	//Send Rotation iso-value [1]
	GLint iValLoc = glGetUniformLocation(g_glslProgram, "iVal");
	glUniform1f(iValLoc, iVal);
	...
	for (int zi = 0; zi < 100; zi++) {
		for (int yi = 0; yi < 256; yi++) {
			for (int xi = 0; xi < 256; xi++) {
				pixel_coordinate(xi + 1, yi, zi, up, vp);
				pixel_coordinate(xi - 1, yi, zi, um, vm);
				Gx = (getTextureR(up, vp, dataVolume, width, height) - getTextureR(um, vm, dataVolume, width, height)) / 2.f;
				pixel_coordinate(xi, yi + 1, zi, up, vp);
				pixel_coordinate(xi, yi - 1, zi, um, vm);
				Gy = (getTextureR(up, vp, dataVolume, width, height) - getTextureR(um, vm, dataVolume, width, height)) / 2.f;
				pixel_coordinate(xi, yi, zi + 1, up, vp);
				pixel_coordinate(xi, yi, zi - 1, um, vm);
				Gz = (getTextureR(up, vp, dataVolume, width, height) - getTextureR(um, vm, dataVolume, width, height)) / 2.f;
				Nx = -Gx / sqrt(Gx*Gx + Gy * Gy + Gz * Gz);
				Ny = -Gy / sqrt(Gx*Gx + Gy * Gy + Gz * Gz);
				Nz = -Gz / sqrt(Gx*Gx + Gy * Gy + Gz * Gz);
				Nx = 255 * (Nx + 1) * 0.5;
				Ny = 255 * (Ny + 1) * 0.5;
				Nz = 255 * (Nz + 1) * 0.5;
				pixel_coordinate(xi, yi, zi, pcu, pcv);
				setTextureR(pcu, pcv, dataNormals, width, height, (int)Nx);
				setTextureG(pcu, pcv, dataNormals, width, height, (int)Ny);
				setTextureB(pcu, pcv, dataNormals, width, height, (int)Nz);
			}
		}
	}
